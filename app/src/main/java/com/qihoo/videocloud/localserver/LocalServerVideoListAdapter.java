...
import com.qihoo.videocloud.utils.ThreadUtils;
...
public class LocalServerVideoListAdapter extends BaseAdapter {
    ...
    private void startPlay(int position, String url, String resId, final QHVCTextureView playView) {
        ...
        try {
            ...
            options.put(IQHVCPlayerAdvanced.KEY_OPTION_RESOURCE_ID, resId);
            mQHVCPlayer.setDataSource(IQHVCPlayer.PLAYTYPE_VOD, newUrl, mCid, options);
        } catch (Exception e) {
                    ...
                    ThreadUtils.runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            if (mQHVCPlayer != null && mCurViewHolder != null && mCurViewHolder.playView != null) {
                                mCurViewHolder.playView.setVideoRatio(mQHVCPlayer.getRenderMode(), (float) width / (float) height);
                            }
                        }
                    });
                }
